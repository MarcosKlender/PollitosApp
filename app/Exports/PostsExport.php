...
class PostsExport implements FromView
{
    ...
    public function view(): View
    {
       $datoid=$this->id;
        ...
        return view('reportes.excelviews.lotdetexcel',[ 'lotes' => Lotes::where('id', $this->id)->get()],['registros' => Registros::where('lotes_id', $this->id)->get()])->with('id',$datoid)->with('total_cantidad',$total_cantidad)->with('total_bruto',$total_bruto)->with('total_gavetas',$total_gavetas)->with('total_final',$total_final)->with('totalv_bruto',$totalv_bruto)->with('totalv_gavetas',$totalv_gavetas)->with('totalv_final',$totalv_final)->with('totale_cantidad',$totale_cantidad)->with('totale_bruto',$totale_bruto)->with('totale_gavetas',$totale_gavetas)->with('totale_final',$total_final)->with('visceras',$viceras)->with('egresos',$egresos);
