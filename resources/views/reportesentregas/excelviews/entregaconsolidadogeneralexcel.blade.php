<table >
     <?php  $ePeso_bruto=0; $tot_pbruto_presas = 0; $total_neto = 0 ?>
     @foreach($entregas as $entrega)  
    <thead>
        <tr> 
            <th align="left" width="30"><b> Entrega N°  {{ $entrega->id }} </b></th>
            <td width="35"></td>
            <td width="15"></td>
            <td width="30"></td>
            <td width="20"></td>
            <td width="20"></td>
        </tr>
    </thead>
    <tbody>
               <tr>
                    <th align="left" ><b>Fecha:</b></th>
                    <td align="right">{{ $entrega->created_at }}</td>
                </tr>
                <tr>
                    <th align="left"><b>Tipo animal:</b></th>
                    <td align="right">{{ $entrega->tipo}} </td>
                </tr>
                <tr>
                    <th align="left"><b>Cliente:</b></th>
                    <td align="right">{{ $entrega->cliente}}</td>
                </tr>
                <tr>
                    <th align="left"><b>Ruc/CI:</b></th>
                    <td align="right">{{ $entrega->ruc_ci }} </td>
                </tr>
                <tr>
                    <th align="left"><b>Cantidad Animales:</b></th>
                    <td align="right">{{ $entrega->cant_animales }}</td>
                </tr>
                <tr>
                    <th align="left"><b>Estado:</b></th>
                    <td align="right"> {{ $liquidado }} </td>
                </tr>
                <tr>
                    <th align="left"><b>Para Local</b></th>
                    <td align="right">
                        @if( $entrega->tipo_entrega == 1 ) 
                            Si 
                        @else 
                            No
                          @endif 
                    </td>
                </tr>  
                <tr></tr>
                <tr>
                    <th align="center" colspan="3"><b>REGISTRO PESOS</b></th>  
                </tr>
                <tr>
                    <th align="right" colspan="2"><b>Cantidad Gavetas:</b></th>
                    @foreach( $eCantidad_gavetas as $eCantidad_gaveta)
                        @if( $eCantidad_gaveta->entregas_id == $entrega->id)
                            <td align="center">{{ $eCantidad_gaveta->cant_gavetas }}</td>
                        @endif
                    @endforeach
                </tr>
                <tr>
                    <th align="right" colspan="2"><b>Peso Bruto (PB):</b></th>
                    @foreach( $PB as $pb)
                      @if( $pb->entregas_id == $entrega->id)
                            <td align="center">{{ $pb->peso_bruto }}</td>
                        {{ $ePeso_bruto =  $pb->peso_bruto }}
                      @endif 
                    @endforeach
                </tr>
                <tr></tr>
                <tr>
                    <th align="center" colspan="3"><b>PESO DE PRESAS</b></th>
                </tr>
                <tr>
                    <th align="center" ><b>Tipo</b></th>  
                    <th align="center" ><b>Cantidad Gavetas</b></th>
                    <th align="center" ><b>Peso (PP)</b></th>  
                </tr> 
                    @foreach($grupo_presas as $presa)
                        @if($presa->entregas_id == $entrega->id)
                                <tr>            
                                    <td align="left">{{ $presa->tipo_entrega }}</td>
                                    <td align="center">{{ $presa->cant_gavetas }}</td>
                                    <td align="center">{{ $presa->peso_bruto }}  </td>
                                </tr>
                         @endif       
                    @endforeach 
                <tr>
                    <th align="center"><b>TOTAL</b></th>
                    @foreach($Total_cgaveta_presas as $tcg)
                        @if($tcg->entregas_id == $entrega->id)
                        <td align="center"><b> {{ $tcg->cant_gavetas }}</b></td>
                        <td align="center"><b>{{ $tcg->peso_bruto }}</b></td>
                            {{ $tot_pbruto_presas = $tcg->peso_bruto }}
                        @endif
                    @endforeach
                </tr>
                <tr>
                </tr>
                <tr>
                    <th align="center" colspan="2"><b>TOTAL NETO(PB+PP)</b></th>
                    <td align="center"><b>{{  $total_neto = $ePeso_bruto + $tot_pbruto_presas }}</b></td>
                </tr><tr>
                </tr><tr>
                    <th align="center"><b>CANTIDAD GAVETAS</b></th>
                    <th align="center"><b>CATEGORIA ANIMAL</b></th>
                </tr>
                @foreach($categoria_animal as $cat_animal)
                     @if($cat_animal->entregas_id == $entrega->id)
                    <tr>
                            <td align="center">{{ $cat_animal->cant_gavetas }}</td>
                            <td align="center">{{ $cat_animal->categoria_animales }}</td>
                    </tr>
                   @endif
                @endforeach
                <tr></tr>
                <tr></tr>
                <tr></tr>

    </tbody>   
    @endforeach  
</table>